    ...
    def __init__(
        ...
        if normalize:
            self.train_transforms = A.Compose([
                ...
                A.Cutout(num_holes=3, max_h_size=8, max_w_size=8, fill_value=self.mean, always_apply=False, p=0.8),
#                 A.CoarseDropout(
...
#                 A.Rotate(limit=5, interpolation=1, border_mode=4, value=None, mask_value=None, always_apply=False, p=0.5),
    ...
    def get_loader(self, batch_size=128):
        ...
        print(
            f"""[INFO] {'train' if self.train else 'test'} dataset of size {len(data)} loaded... (v4.3)"""
        )
        return data_loader
